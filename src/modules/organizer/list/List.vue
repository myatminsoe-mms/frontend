      ...
      >
          ...
          <div class="flex flex-wrap justify-content-between align-items-center">
            ...
            <span class="p-input-icon-left">
              <div class="mt-3 md:mt-0">
                ...
                <router-link v-if="$can('create', 'User')" :to="{ name: 'newOrganizer' }">
        ...
        <Column v-for="column in selectedColumns" :key="column.field" :field="column.field" :header="column.header" :sortable="column.sortable" :frozen="!isMobile && column.frozen" :alignFrozen="column.alignFrozen" :style="column.style">
          <template v-if="column.field === 'actions'" #body="{ data }">
            <div class="flex">
              <router-link v-if="$can('update', 'Organizer')" :to="{ name: 'editOrganizer', params: { id: data.id } }">
                <Button type="button" icon="pi pi-pencil" class="mr-2" text rounded />
              </router-link>
              <Button v-if="$can('delete', 'Organizer')" type="button" icon="pi pi-trash" class="p-button-danger" text rounded @click="showConfirmDialog(data.id)" />
            </div>
          </template>
        </Column>
      </DataTable>
...
const { dt, lazyParams, totalRecords, records, isLoading, search, actionItems, actionMenu, toggleActionMenu, showConfirmDialog, onPage, onSort, toggleColumnMenu, selectedColumns, columns, columnMenu, selectedStatus, statuses } = useList()
